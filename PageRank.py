...
def PageRank(input_file_name, damping_factor=0.85, max_iter=100):
    '''
    Vanilla PageRank
    '''

    # 读入文件
    nodes, forward, backward = read_graph(input_file_name)

    N = len(nodes)
    page_rank = dict.fromkeys(nodes, 1.0 / N)
    damping_val = (1.0 - damping_factor) / N
...
_ = PageRank('soc-Epinions-after.txt')
